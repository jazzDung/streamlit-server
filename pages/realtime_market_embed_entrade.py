...
order by  indexed_timestamp_ desc;
    ...
    df['buy_minus_sell_count'] = df['net_buy_count'] - df['net_sell_count']
    return df


def load_buy_sell_v2(timestamp: datetime):
    symbol_sql = f"""
        select *
        from public.symbol_all_metric
    ...
    sector['net_sell_count'] = symbol.groupby('sector')['net_sell'].apply(lambda x: (x > 0).sum()).reset_index()[
        'net_sell']
    ...
    sector['foreign_trade_count'] = sector['net_buy_count'] + sector['net_sell_count']
    ...
    sector['net_buy_ratio_without_neutral'] = sector['net_buy_count'] / sector['foreign_trade_count']
    sector['net_sell_ratio_without_neutral'] = sector['net_sell_count'] / sector['foreign_trade_count']
    return sector
...
def get_liquidity_and_min_max_5_day(timestamp: datetime):
    ...
    df['time'] = df['indexed_timestamp_'].dt.strftime('%H:%M')
...
def get_top_n_sector_liquidity(timestamp: datetime, n=8):
    ...
    top_8_liquidity = \
    df[df['indexed_timestamp_'] == df['indexed_timestamp_'].max()].sort_values(by='liquidity', ascending=False).head(n)[
        'name'].unique().tolist()
    today = df[df['indexed_timestamp_'].dt.date == timestamp.date()]
...
price_change_ratio = price_change_ratio.sort_values(
    by='symbol_count',
    ascending=False
).head(8)
...
net_buy_sell = net_buy_sell.sort_values(by='symbol_count', ascending=False).head(8)
# net_buy_sell = net_buy_sell.sort_values(by='net_buy_ratio', ascending=False).head(8)
...
net_buy_sell = load_buy_sell_v2(datetime(2023, 12, 12))
top_net_buy = net_buy_sell.sort_values(by='net_value', ascending=False).head(5)

fig_top_net_buy = px.scatter(
    data_frame=top_net_buy,
    x="net_value",
    # y="net_buy_ratio",
    y="net_buy_ratio_without_neutral",
    color="sector",
    size="net_buy_count",
    labels={
        "sector": "Ngành cấp 3",
        "net_buy_count": "Số lượng mã mua ròng",
        "net_buy_ratio": "Tỉ lệ mã mua ròng",
        "net_buy_ratio_without_neutral": "Tỉ lệ mã mua ròng",
        "net_value": "Tổng khối lượng mua ròng"
    },
    height=600
).update_traces(
    marker=dict(
...
net_buy_sell = load_buy_sell_v2(datetime(2023, 12, 12))
top_net_sell = net_buy_sell.sort_values(by='net_value').head(5)

fig_top_net_sell = px.scatter(
    data_frame=top_net_sell,
    x="net_value",
    # y="net_sell_ratio",
    y="net_sell_ratio_without_neutral",
    color="sector",
    size="net_sell_count",
    # text="nganhcap3",
    labels={
        ...
        "net_sell_count": "Số lượng mã bán ròng",
        "net_sell_ratio": "Tỉ lệ mã bán ròng",
        "net_sell_ratio_without_neutral": "Tỉ lệ mã bán ròng",
        "net_value": "Tổng khối lượng bán ròng"
    },
    height=600
).update_traces(
    marker=dict(
